...
try {
    ...
    if ($selectedDepartmentId !== null && $selectedDepartmentId > 0) {
        ...
        $stmtStaff = $pdo->prepare(
            "SELECT s.id, s.department_id, s.name, s.designation, s.subject_discipline, s.photo_url, s.profile_details, s.is_teaching_staff, s.IsActive
             FROM staff s
             JOIN departments d ON s.department_id = d.id
             JOIN universities u ON d.university_id = u.id
             WHERE s.department_id = :department_id
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($member['photo_url'])): ?>
                                            <img src="images/Staff_Photos/Nursing/<?php echo htmlspecialchars($member['photo_url']); ?>" alt="Photo of <?php echo htmlspecialchars($member['name']); ?>" class="staff-photo" onerror="this.onerror=null;this.src='https://placehold.co/48x48/cccccc/333333?text=N/A';">
                                        <?php else: ?>
                                            N/A
                                        <?php endif; ?>
                                    </td>
