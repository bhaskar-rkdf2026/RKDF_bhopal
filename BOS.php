...
$eyebrow      = !empty($pRow['eyebrow'])       ? $pRow['eyebrow']       : '18 · BOARD OF STUDIES';
$mainTitle    = !empty($pRow['page_title'])    ? $pRow['page_title']    : 'Board of Studies (BOS)';
$heroSubtitle = !empty($pRow['hero_subtitle']) ? $pRow['hero_subtitle'] : 'Departmental bodies recommending syllabus revisions, textbook lists, and panel of examiners.';
$introText    = !empty($pRow['intro_text'])    ? $pRow['intro_text']    : 'Each department has a Board of Studies that meets annually to modernize course syllabi in line with NEP-2020 and industry demands.';
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    .bos-block-card { background: #ffffff; border: 1px solid rgba(12,20,36,0.08); border-radius: 20px; overflow: hidden; box-shadow: 0 4px 24px rgba(12,20,36,0.04); margin-bottom: 36px; }
    .bos-card-header { background: #0C1424; color: #ffffff; padding: 24px 32px; display: flex; align-items: center; justify-content: space-between; border-bottom: 3px solid #C5A059; }
    .bos-card-title { font-family: 'Playfair Display', Georgia, serif; font-size: 24px; font-weight: 700; color: #ffffff; margin: 0; }
    .bos-card-body { padding: 36px 32px; }
  ...
  <main class="bos-main-section">
    <div class="rk-container">
      <div class="bos-grid-layout">
        ...
        <!-- LEFT COLUMN -->
        <div>
          <article class="bos-block-card">
            <div class="bos-card-header">
              <h2 class="bos-card-title"><?= htmlspecialchars($mainTitle) ?> Overview</h2>
              <span style="background:rgba(197,160,89,0.18);color:#C5A059;padding:4px 12px;border-radius:99px;font-size:11px;font-weight:700;">CURRICULUM</span>
            </div>
