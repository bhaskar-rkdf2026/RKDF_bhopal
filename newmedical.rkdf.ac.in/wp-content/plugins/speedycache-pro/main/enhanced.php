...
class Enhanced{
	...
	static function preload_critical_images($content){
		...
		foreach($images as $image){
			...
			if(!empty($speedycache->options['critical_image_count']) && $count >= $speedycache->options['critical_image_count']){
				break;
			}
			...
			if(strpos($preload_tags, $atts['src']) === FALSE){
				$preload_tags .= '<link rel="preload" as="image" '. (empty($preload_tags) ? ' fetchpriority="high"' : '') .' href="'.esc_attr($atts['src']).'"'. (!empty($atts['srcset']) ? ' imagesrcset="'. esc_attr($atts['srcset']).'"' : '') . (!empty($atts['sizes']) ? ' imagesizes="'.esc_attr($atts['sizes']).'"' : '') . ' />';
			}
	...
	static function preload_resource(){
		...
		$permalink = (is_int($post_id) && $post_id > 0) ? get_permalink($post_id) : '';
		...
		foreach($speedycache->options['preload_resource_list'] as $preload_resource){
			...
			if(!empty($preload_resource['pages']) && is_array($preload_resource['pages'])){
				if(!empty($permalink) && !in_array($permalink, $preload_resource['pages'])){
					continue;
				}
			}
