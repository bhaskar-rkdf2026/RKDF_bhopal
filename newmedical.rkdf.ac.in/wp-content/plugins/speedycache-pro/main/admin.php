...
// For Cloudflare Rule
add_action('speedycache_after_cdn_save', 'speedycache_pro_cloudflare_rule_init');
...
add_action('wp_ajax_speedycache_pro_update_cf_rules', 'speedycache_pro_update_cf_rules');
...
// Initializes the core process to automatically set up, update, or delete the Cloudflare cache rule when CDN settings are saved.
function speedycache_pro_cloudflare_rule_init(){
	global $speedycache;

	if(!class_exists('\SpeedyCache\CloudflareRule')){
		return;
	}
	
	// If we dont have a key we cant do any action, create/delete
	if(empty($speedycache->cdn['cdn_key']) || $speedycache->cdn['cdn_type'] !== 'cloudflare'){
		return;
	}

	$cdn_enabled = isset($_POST['enable_cdn']);
	$cloudflare_enabled = isset($_POST['enabled_cloudflare']);

	if(!empty($speedycache->cdn['enabled_cloudflare']) && !empty($speedycache->cdn['enabled'])){
		$error = '';
		$rule = \SpeedyCache\CloudflareRule::setup_cache_rule($error);

		if(\SpeedyCache\CloudflareRule::get_last_error_code() == 10000){
			$speedycache->cdn['error'] = $error;
			return;
		}
	}

	// Delete rule on settings save
	if(!$cdn_enabled || !$cloudflare_enabled){
		\SpeedyCache\CloudflareRule::maybe_rule_delete($cdn_enabled, $cloudflare_enabled);
	}
}

// Re-syncs the Cloudflare cache rule when the speedycache_exclude option is updated.
function speedycache_pro_cloudflare_exclude_updated($old_value, $new_value){
	...
	$cdn_options = get_option('speedycache_cdn', []);

	if(
		empty($cdn_options['enabled_cloudflare']) || 
		empty($cdn_options['enabled']) || 
		empty($cdn_options['cdn_key']) || 
		$cdn_options['cdn_type'] !== 'cloudflare'
	){
		return;
	}
	...
	// Drop the cached ruleset so setup_cache_rule() re-fetches and re-evaluates with fresh excludes.
	\SpeedyCache\CloudflareRule::$cached_ruleset = [];

	$error = '';
	\SpeedyCache\CloudflareRule::setup_cache_rule($error);

	if(defined('WP_DEBUG') && WP_DEBUG && !empty($error)){
		error_log('[SpeedyCache Pro] Cloudflare rule resync after excludes update failed: ' . $error);
	}
}

function speedycache_pro_update_cf_rules(){
