...
$eyebrow      = !empty($pRow['eyebrow'])       ? $pRow['eyebrow']       : '09 · ADMINISTRATION';
$mainTitle    = !empty($pRow['page_title'])    ? $pRow['page_title']    : 'Director General Management (DGM) Profile';
$heroSubtitle = !empty($pRow['hero_subtitle']) ? $pRow['hero_subtitle'] : 'Overseeing operational management, strategic planning, and campus administrative workflows.';
$introText    = !empty($pRow['intro_text'])    ? $pRow['intro_text']    : 'The DGM office ensures seamless day-to-day administrative execution, student services, and facility management across campus.';
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    .dgm-block-card { background: #ffffff; border: 1px solid rgba(12,20,36,0.08); border-radius: 20px; overflow: hidden; box-shadow: 0 4px 24px rgba(12,20,36,0.04); margin-bottom: 36px; }
    ...
    .dgm-card-title { font-family: 'Playfair Display', Georgia, serif; font-size: 24px; font-weight: 700; color: #ffffff; margin: 0; }
        ...
        <div>
          <article class="dgm-block-card">
            <div class="dgm-card-header">
              <h2 class="dgm-card-title">DGM Profile &amp; Desk</h2>
              <span style="background:rgba(227,27,35,0.18);color:#E31B23;padding:4px 12px;border-radius:99px;font-size:11px;font-weight:700;">DGM OFFICE</span>
            </div>
