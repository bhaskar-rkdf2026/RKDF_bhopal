<?php
// ============================================================
// RKDF University — Faculty Deans Directory (100% Dynamic CMS)
...
$eyebrow      = !empty($pRow['eyebrow'])       ? $pRow['eyebrow']       : '13 · ACADEMIC LEADERSHIP';
$mainTitle    = !empty($pRow['page_title'])    ? $pRow['page_title']    : 'Faculty Deans Directory';
$heroSubtitle = !empty($pRow['hero_subtitle']) ? $pRow['hero_subtitle'] : 'Directory of Deans leading constituent faculties of Engineering, Pharmacy, Management, Science, Law, Agriculture, and Paramedical across RKDF University Bhopal.';

$defaultMessage = "The Deans of Faculties at RKDF University, Bhopal provide academic vision, curriculum governance, research supervision, and departmental leadership across all university faculties.\n\nWorking in close coordination with Institute Heads and Department Heads (HODs), our Faculty Deans ensure state-of-the-art education, multidisciplinary research, accreditation standards, and student excellence.";

$introHeading = !empty($pRow['intro_heading']) ? $pRow['intro_heading'] : "Deans of University Faculties";
...
<head>
  ...
  <style>
    ...
    .dean-main-section { padding: 80px 0 100px; background: #FAF9F5; color: #0C1424; }
    .dean-grid-layout { display: grid; grid-template-columns: 8.5fr 3.5fr; gap: 48px; align-items: start; }
    @media (max-width: 992px) { .dean-grid-layout { grid-template-columns: 1fr; } }
    ...
    .dean-intro-title { font-family: 'Playfair Display', Georgia, serif; font-size: 26px; font-weight: 700; color: #0C1424; margin-bottom: 12px; }
    .dean-intro-text { font-size: 16.5px; line-height: 1.8; color: #334155; margin: 0; }
  ...
  <main class="dean-main-section">
    <div class="rk-container">
      <div class="dean-grid-layout">
        ...
        <div>
          <div class="dean-intro-card">
            <h2 class="dean-intro-title"><?= htmlspecialchars($introHeading) ?></h2>
            <?php
            $introParas = explode("\n", $introText);
            foreach ($introParas as $ipara):
              ...
              if (!empty($itrim)):
            ?>
            <p class="dean-intro-text"><?= htmlspecialchars($itrim) ?></p>
            <?php
              endif;
            endforeach;
            ?>
          ...
          <div class="dean-list-grid">
            <?php foreach ($allItems as $item): ?>
            <article class="dean-member-card">
              <div class="dean-avatar-box">
                <img src="<?= htmlspecialchars(!empty($item['image_path']) ? $item['image_path'] : 'images/lovable/rkdf-logo.png') ?>" alt="<?= htmlspecialchars($item['title']) ?>" class="dean-img" onerror="this.src='images/lovable/rkdf-logo.png';">
              </div>
              <h3 class="dean-name"><?= htmlspecialchars($item['title']) ?></h3>
              <div><span class="dean-desig-badge"><?= htmlspecialchars($item['badge_text'] ?: 'FACULTY DEAN') ?></span></div>
              <div class="dean-faculty-title"><?= htmlspecialchars($item['subtitle']) ?></div>
        ...
        <aside>
          <div class="sidebar-card">
            ...
            <ul class="sidebar-nav-list">
              ...
              <li><a href="dean.php" class="sidebar-link active"><span>Faculty Deans</span> <span>↗</span></a></li>
