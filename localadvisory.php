...
$eyebrow      = !empty($pRow['eyebrow'])       ? $pRow['eyebrow']       : '20 · LOCAL ADVISORY';
$mainTitle    = !empty($pRow['page_title'])    ? $pRow['page_title']    : 'Local Core Advisory Group';
$heroSubtitle = !empty($pRow['hero_subtitle']) ? $pRow['hero_subtitle'] : 'Regional leaders and industrial experts supporting community engagement and local industry tie-ups.';
$introText    = !empty($pRow['intro_text'])    ? $pRow['intro_text']    : 'Connecting academic projects with local industry needs, rural extension activities, and community welfare in MP.';
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    .la-block-card { background: #ffffff; border: 1px solid rgba(12,20,36,0.08); border-radius: 20px; overflow: hidden; box-shadow: 0 4px 24px rgba(12,20,36,0.04); margin-bottom: 36px; }
    .la-card-header { background: #0C1424; color: #ffffff; padding: 24px 32px; display: flex; align-items: center; justify-content: space-between; border-bottom: 3px solid #C5A059; }
    .la-card-title { font-family: 'Playfair Display', Georgia, serif; font-size: 24px; font-weight: 700; color: #ffffff; margin: 0; }
    .la-card-body { padding: 36px 32px; }
  ...
  <main class="la-main-section">
    <div class="rk-container">
      <div class="la-grid-layout">
        ...
        <!-- LEFT COLUMN -->
        <div>
          <article class="la-block-card">
            <div class="la-card-header">
              <h2 class="la-card-title"><?= htmlspecialchars($mainTitle) ?></h2>
              <span style="background:rgba(197,160,89,0.18);color:#C5A059;padding:4px 12px;border-radius:99px;font-size:11px;font-weight:700;">REGIONAL</span>
            </div>
        ...
        <aside>
          <div class="sidebar-card">
            <h3 class="sidebar-title">Governance &amp; Advisory</h3>
            <ul class="sidebar-nav-list">
              <li><a href="Statuary-Bodies.php" class="sidebar-link">National Advisory <span>→</span></a></li>
              <li><a href="localadvisory.php" class="sidebar-link active">Local Advisory <span>→</span></a></li>
